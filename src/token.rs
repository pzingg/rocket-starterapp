...
impl UserToken {
    ...
    pub fn as_anonymous_string(&self) -> String {
        let anonymous_token = UserToken { uidb64: None, ts: self.ts.clone(), token: self.token.clone() };
        anonymous_token.to_string()
    }
}
...
impl fmt::Display for UserToken {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.uidb64 {
            Some(uid) => write!(f, "{}-{}-{})", uid, self.ts, self.token),
            None => write!(f, "{}-{}", self.ts, self.token),
        }
    }
}

impl FromParam<'_> for UserToken {
    ...
    fn from_param(param: &str) -> Result<Self, Self::Error> {
        ...
        Err(error::Error::with_status(anyhow!("Invalid token: {}", param), Status::BadRequest))
    }
}
...
/// KEY_SALT values.
fn hash(value: &str, timestamp: u64) -> error::Result<UserToken> {
    ...
    // TODO: Use Figment to specify the key and salt.
    let secret_key = env::var("SECRET_KEY")
        .expect("Unable to pull SECRET_KEY for account token generation");
    let key_salt = env::var("KEY_SALT")
        .unwrap_or_else(|_| KEY_SALT.to_owned());

    let key = format!("{}{}", key_salt, secret_key);
    let mut hasher = HmacSha256::new_from_slice(key.as_bytes())
        .map_err(|e| error::Error::from(anyhow!("Error generating HMACSHA256: {:?}", e)))?;
    ...
    Ok(UserToken{ uidb64: None, ts: ts.as_str().to_lowercase(), token: hash })
}
...
pub trait OneTimeUseTokenGenerator {
    ...
    /// if not configured).
    fn create_reset_token(&self) -> error::Result<String> {
        let value = self.hash_value();
        let since = num_seconds();
        hash(&value, since as u64).map(|t| t.to_string())
    }
